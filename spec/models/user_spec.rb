...
RSpec.describe User, type: :model do
  ...
  describe 'Instance Methods' do
    ...
    xit 'Merchants who fullfilled the most orders this month' do
      ...
      order_2 = create(:order, user: user, status: 'completed')
      create(:order_item, order: order_2, item: item_3, fulfilled: false)
      create(:order_item, order: order_2, item: item_4, fulfilled: false)

      order_3 = create(:order, user: user, status: 'completed')
      create(:order_item, order: order_3, item: item_3,quantity: 1000, fulfilled: false)
      create(:order_item, order: order_3, item: item_4, fulfilled: false)
      order_3.update(created_at: 101.days.ago, updated_at: 100.days.ago)

      order_4 = create(:order, user: user, status: 'completed')
      create(:order_item, order: order_4, item: item_5,quantity: 1000, fulfilled: true)
      create(:order_item, order: order_4, item: item_6, fulfilled: true)

      order_5 = create(:order, user: user, status: 'completed')
      create(:order_item, order: order_5, item: item_5,quantity: 1000, fulfilled: true)
